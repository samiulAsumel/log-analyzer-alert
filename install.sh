#!/usr/bin/env bash
# install.sh — One-command setup for Automated Log Analyzer & Alert System v2.0.0
# Installs scripts, configures cron jobs or systemd timers, creates directories.
# Usage: sudo bash install.sh [--uninstall] [--dry-run] [--systemd]
set -euo pipefail
...
DRY_RUN=false; USE_SYSTEMD=false
for _arg in "$@"; do
    case "$_arg" in
...
# Auto-detect systemd if available and --systemd not explicitly set
if ! $USE_SYSTEMD && command -v systemctl &>/dev/null && systemctl is-system-running &>/dev/null 2>&1; then
    USE_SYSTEMD=true
fi

$DRY_RUN && echo -e "${YELLOW}${BOLD}[DRY-RUN] No changes will be made${RESET}\n"
$USE_SYSTEMD && echo -e "${CYAN}[INFO] Systemd detected — will install timers instead of cron${RESET}"
...
if $DRY_RUN; then
    ...
    for mod in alert_engine analyze_auth analyze_system analyze_nginx \
               analyze_mysql analyze_app analyze_journal detect_threats; do
    ...
    if [[ -d "${SRC_DIR}/scripts" ]]; then
        ...
        ok "Installed utility scripts → ${INSTALL_DIR}/scripts/"
    fi
...
# ── Set up scheduling (systemd timers or cron) ────────────────────────────────
step "Setting up scheduling"

if $USE_SYSTEMD; then
    SYSTEMD_UNIT_DIR="/etc/systemd/system"
    if $DRY_RUN; then
        log "[DRY-RUN] Would install systemd units to ${SYSTEMD_UNIT_DIR}"
    else
        for unit in loganalyzer.service loganalyzer.timer \
                    loganalyzer-digest.service loganalyzer-digest.timer; do
            if [[ -f "${SRC_DIR}/systemd/${unit}" ]]; then
                cp "${SRC_DIR}/systemd/${unit}" "${SYSTEMD_UNIT_DIR}/${unit}"
        ...
        systemctl enable --now loganalyzer.timer
        systemctl enable --now loganalyzer-digest.timer
        ok "Systemd timers enabled and started"
        ok "  loganalyzer.timer  — runs every 15 min"
        ok "  loganalyzer-digest.timer — runs daily at 07:00"
    fi
else
...
# Automated Log Analyzer & Alert System — auto-generated by install.sh v2.0.0
SHELL=/bin/bash
...
SCHED_INFO="cron (/etc/cron.d/loganalyzer)"
$USE_SYSTEMD && SCHED_INFO="systemd timers (loganalyzer.timer)"

cat <<INFO

  Config:         ${CONFIG_DIR}/config.conf
  Scripts:        ${INSTALL_DIR}/
  Utility scripts:${INSTALL_DIR}/scripts/
  State files:    ${STATE_DIR}/
  Logs:           ${LOG_DIR}/analyzer.log
  Findings:       ${STATE_DIR}/findings/
  JSON findings:  ${STATE_DIR}/findings_json/
  Scheduling:     ${SCHED_INFO}

  NEXT STEPS:
  ─────────────────────────────────────────────────────────────
  1. Edit config:    nano ${CONFIG_DIR}/config.conf
     Set: ALERT_EMAIL, SLACK_WEBHOOK, TEAMS_WEBHOOK, PAGERDUTY_KEY
  ...
  8. Health check:   bash ${INSTALL_DIR}/scripts/health_check.sh

  9. Export SIEM:    bash ${INSTALL_DIR}/scripts/export_findings.sh --format=ndjson

  10. Uninstall:     sudo bash ${SRC_DIR}/install.sh --uninstall
  ─────────────────────────────────────────────────────────────
